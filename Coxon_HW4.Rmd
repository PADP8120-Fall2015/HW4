...
```
*Looking at the pairwise correlations amongst the variables, it appears that the my most promising variabke to include in my model would be regress percent.taking and teacher.pay on sat.both.*
...
*It appears that forward selection using the AIC/BIC also reveal that the optimal model is still m4 with the variables percent.taking and percent.no.hs regressed on sat.both.*
$E(sat.both|X_1,X_2)$ = $\beta_0$ + $\widehat{\beta}_percent.taking* x_1$  + $\widehat{\beta}_percent.no.hs* x_2$ 
